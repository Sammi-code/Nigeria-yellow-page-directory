...
function ManageAds() {
    ...
    for (i = 0; i < userInfo.length; i++) {
        ...
        backdrop-filter: blur(55.63px);border-radius: 10px;">
            ...
            <p style= "font-family:ubuntu;color: #F8C800; font-size: 18px; font-weight: 500; line-height: 25px;">Information : <span style="color:#fff "> Number of adverts determined by your package plan </span></p>
            </div>
            </div>
            ...
            <button onclick="displayAds()" style="width:180px; color:white; margin-left:30px; height:53px; border:none; border-radius:10px; background-color:#F8C810 !important; padding:1px !important;  box-shadow:#686868; font-weight:bolder;" type="button" id= "main-btn" class="btn btn-primary" >
                View Adverts
            </button>
              <br><br>
...
function adCreate(){
  ...
  newAd = {
      ...
      "image" : imgSrc,
      "adText" : document.getElementById("desc").value
  }
       if(newAd.adName != '' || newAd.adText != '' && adsPost.length != 0){
...
function displayAds(){
  if(adsPost.length == [] ){
    swal("", "You have not posted any Adverts", "error")
}

  userDetails = ' <p style="color:#F8C800; font-size:18px; cursor:pointer;" onclick="ManageAds()"><i class="fas fa-long-arrow-alt-left"></i> Go back to upload page</p><br><br> ';
  for (i = 0; i < adsPost.length; i++) {
      userDetails = `

      <p style="color:#F8C800; font-size:18px; cursor:pointer;" onclick="ManageAds()"><i class="fas fa-long-arrow-alt-left"></i> Go back to upload page</p><br><br> 
  <div class="card mb-3" id="forza">
  <div class="row no-gutters">
    <div class="col-md-4" id="marko">
      <img src="${adsPost[i].image}" class="card-img" alt="...">
    </div>
    <div class="col-md-8">
      <div class="card-body">
        <h5 class="card-title">${adsPost[i].adName}</h5>
        <p class="card-text">${adsPost[i].adText}</p>
        <p class="card-text"><strong class="text-muted">${userInfo[i].businessName}</strong></p>
        <p class="card-text"><small class="text-muted">Last updated 3 mins ago</small></p>
        <button type="button" onclick="deleteAds(${i})" class="btn btn-primary">Delete</button>
      </div>
    </div>
  </div>
</div>
